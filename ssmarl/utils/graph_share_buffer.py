import torch
import numpy as np
from ssmarl.utils.util import get_shape_from_act_space
...
def _flatten_2d_n_1(x):
    return x[:-1].reshape(-1, *x.shape[2:])

def _flatten_2d(x):
    return x.reshape(-1, *x.shape[2:])

class GraphShareReplayBuffer(object):
    def __init__(self, args, graph_obs_shape, act_space):
        ...
        # ------------------------- Observation Related -------------------------
        self.agent_id = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, 1), dtype=np.int32)
        ...
        # ------------------------- RNN Related -------------------------
        self.rnn_states = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, self.recurrent_N, self.rnn_hidden_size), dtype=np.float32)
        self.rnn_states_critic = np.zeros_like(self.rnn_states)
        # multiple costs
        self.rnn_states_costs = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, self.num_costs, self.recurrent_N, self.rnn_hidden_size), dtype=np.float32)
        ...
        self.value_preds = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, 1), dtype=np.float32)
        self.returns = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, 1), dtype=np.float32)

        # ------------------------- Costs Related -------------------------
        self.costs = np.zeros((self.episode_length, self.n_rollout_threads, self.num_agents, self.num_costs), dtype=np.float32)
        self.cost_preds = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, self.num_costs), dtype=np.float32)
        self.cost_returns = np.zeros((self.episode_length + 1, self.n_rollout_threads, self.num_agents, self.num_costs), dtype=np.float32)
    ...
    def insert(self, 
        ...
        self.rnn_states_costs[self.step + 1] = rnn_states_costs.copy()
    ...
    def after_update(self):
        ...
        self.rnn_states_costs[0] = self.rnn_states_costs[-1].copy()
    ...
    def feed_forward_generator(self, 
        ...
        if mini_batch_size is None:
            assert batch_size >= num_mini_batch, ("".format(n_rollout_threads, episode_length, n_rollout_threads * episode_length, num_mini_batch))
            mini_batch_size = batch_size // num_mini_batch
        else:
            assert num_mini_batch * mini_batch_size <= episode_length * n_rollout_threads, ("num_mini_batch {} * mini_batch_size {} > episode_length {} * n_rollout_threads {}".format(num_mini_batch, mini_batch_size, episode_length, n_rollout_threads))

        rand = torch.randperm(batch_size).numpy()
        sampler = [rand[i*mini_batch_size:(i+1)*mini_batch_size] for i in range(num_mini_batch)]

        agent_id = _flatten_2d_n_1(self.agent_id)
        nodes_feats = _flatten_2d_n_1(self.nodes_feats)
        edge_index = _flatten_2d_n_1(self.edge_index)
        edge_attr = _flatten_2d_n_1(self.edge_attr)

        rnn_states = _flatten_2d_n_1(self.rnn_states)
        rnn_states_critic = _flatten_2d_n_1(self.rnn_states_critic)
        rnn_states_costs = _flatten_2d_n_1(self.rnn_states_costs)
        actions = _flatten_2d(self.actions)
        action_log_probs = _flatten_2d(self.action_log_probs)
        if self.available_actions is not None:
            available_actions = self.available_actions[:-1].reshape(-1, self.available_actions.shape[-1])
        value_preds = _flatten_2d_n_1(self.value_preds)
        returns = _flatten_2d_n_1(self.returns)
        cost_preds = _flatten_2d_n_1(self.cost_preds)
        cost_returns = _flatten_2d_n_1(self.cost_returns)
        masks = _flatten_2d_n_1(self.masks)
        active_masks = _flatten_2d_n_1(self.active_masks)
        ...
        for indices in sampler:
            ...
            rnn_states_costs_batch = rnn_states_costs[indices]
            ...
            if self.factor is None:
                yield nodes_feats_batch, edge_index_batch, edge_attr_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch
            else:
                factor_batch = factor[indices]
                yield agent_id_batch, nodes_feats_batch, edge_index_batch, edge_attr_batch,  rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch, cost_preds_batch, cost_return_batch, rnn_states_costs_batch, cost_adv_targ
    ...
    def recurrent_generator(self,
        ...
        agent_id = self.agent_id[:-1].transpose(1, 0, 2, 3).reshape(-1, *self.agent_id.shape[2:])
        ...
        rnn_states_cost = self.rnn_states_costs[:-1].transpose(1, 0, 2, 3, 4, 5).reshape(-1, *self.rnn_states_costs.shape[2:])
        ...
        for indices in sampler:
            ...
            rnn_states_cost_batch = np.stack(rnn_states_cost_batch).reshape(N, *self.rnn_states_costs.shape[2:])
            ...
            yield agent_id_batch, nodes_feats_batch, edge_index_batch, edge_attr_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, value_preds_batch, return_batch, masks_batch, active_masks_batch, old_action_log_probs_batch, adv_targ, available_actions_batch, factor_batch, cost_preds_batch, cost_return_batch, rnn_states_cost_batch, cost_adv_targ
